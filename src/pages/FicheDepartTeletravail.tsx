...
const ficheSchema = z.object({
  description: z.string().trim().max(5000, { message: "La description ne doit pas dépasser 5000 caractères" }).optional().refine(
    (val) => !val || !/[<>]/.test(val),
...
export default function FicheDepartTeletravail() {
  ...
  const handleSubmit = async (e: React.FormEvent<HTMLFormElement>) => {
    ...
    try {
      ...
      const title = `Départ Télétravail - ${userProfile.full_name} - ${validated.campagne}`;
